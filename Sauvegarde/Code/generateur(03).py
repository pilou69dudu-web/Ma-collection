...
for index, row in df.iterrows():
    ...
    if genre_vinyl == 'nan' or genre_vinyl == '': genre_vinyl = "N/C"
    ...
    <div class="sticky-wrapper">
        <div class="container">
            <div class="search-container">
                <input type="text" id="searchBox" class="search-box" placeholder="Rechercher un artiste, un titre, un pays, un genre...">
                <div class="navigation-filters">
                    ...
                    <div class="filter-row">
                        ...
                        <button class="nav-btn genre-filter active" data-genre="ALL">Tous</button>
                        <button class="nav-btn genre-filter" data-genre="FUNK">Funk</button>
                        <button class="nav-btn genre-filter" data-genre="SOUL">Soul</button>
                        <button class="nav-btn genre-filter" data-genre="DISCO">Disco</button>
                    </div>
                    <div class="filter-row">
...
html_fin = """
        ...
        function renderGrid() {
            let filtered = vinylData.filter(item => {
                const sMatch = !currentSearch || 
                               ...
                               (item.genre && item.genre.toLowerCase().includes(currentSearch)) || 
                               (item.country && item.country.toLowerCase().includes(currentSearch));
                
                const tMatch = currentType === "ALL" || item.type === currentType;
                
                const gMatch = currentGenre === "ALL" || (item.genre && item.genre.toUpperCase().includes(currentGenre));
        ...
        document.querySelectorAll('.genre-filter').forEach(btn => btn.addEventListener('click', e => { document.querySelectorAll('.genre-filter').forEach(b => b.classList.remove('active')); e.target.classList.add('active'); currentGenre = e.target.getAttribute('data-genre'); renderGrid(); }));
...
print("\n🎉 Génération terminée ! Une nouvelle ligne de boutons de tri par 'Genre' (Funk, Soul, Disco...) a été ajoutée !")