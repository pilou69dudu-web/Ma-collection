...
liste_types_uniques = set()  # <--- AJOUT : Pour stocker les types uniques du fichier Excel
...
for index, row in df.iterrows():
    ...
    # AJOUT : On ajoute le type nettoyé à notre set (sauf si vide)
    if valeur_type:
...
# MODIFICATION : Tri des types par ordre alphabétique pour l'affichage des boutons
types_tries = sorted(list(liste_types_uniques))

# =====================================================================
# CRÉATION DU HTML SÉCURISÉ
# =====================================================================
# MODIFICATION : Dans html_debut, la div de la ligne "Type" possède maintenant un id="typeButtonsContainer" et est vidée de ses boutons fixes.
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: sans-serif; }
        ...
        /* Menu déroulant de genre personnalisé moderne */
...
html_fin = """
        ...
        // --- MODIFICATION : GÉNÉRATION DYNAMIQUE DES BOUTONS DE TYPE ---
        ...
        typesAuto.forEach(type => {
            ...
            // Conversion en minuscules avec première lettre en majuscule pour l'esthétique du bouton (ex: COMPILS -> Compils)
            btn.textContent = type.toLowerCase();
            typeContainer.appendChild(btn);
        });
        
        // Remplissage initial de la liste déroulante personnalisée
        ...
        // MODIFICATION : Écouteur d'événement délégué au parent pour fonctionner sur les boutons dynamiques
        ...
        // --- LOGIQUE DU MENU DÉROULANT PERSONNALISÉ ---
        ...
        // Filtrer la liste des genres lorsqu'on écrit dedans
        searchInside.addEventListener('input', (e) => {
            populateGenreOptions(e.target.value);
        });

        // Empêcher la fermeture quand on clique sur la barre de recherche interne
        searchInside.addEventListener('click', (e) => e.stopPropagation());

        // Clic sur une option (Tous les genres ou un genre précis)
        document.addEventListener('click', (e) => {
            ...
            if (option) {
                ...
                searchInside.value = ""; // Réinitialise la recherche interne
                populateGenreOptions();   // Réaffiche la liste complète pour la prochaine fois
                renderGrid();
            } else {
