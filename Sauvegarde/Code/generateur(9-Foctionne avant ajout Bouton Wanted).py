...
import requests
import time
import math # <--- AJOUT : Pour l'arrondi supérieur (math.ceil)
...
# --- AJOUT : FONCTION POUR RÉCUPÉRER LE PRIX DEPUIS DISCOGS ---
def recuperer_prix_haut(release_id):
    ...
    try:
        ...
        if reponse.status_code == 200:
            data = reponse.json()
            # On cherche la suggestion pour l'état Neuf (Mint) ou Très bon (Near Mint)
            prix_brut = None
            if "Mint (M)" in data:
                ...
                # Sécurité : prend la valeur max proposée s'il n'y a pas les clés standard
                prix_brut = max(item["value"] for item in data.values() if isinstance(item, dict) and "value" in item)
            
            if prix_brut:
                return f"{math.ceil(prix_brut)}€" # Arrondi supérieur
        elif reponse.status_code == 429:
            time.sleep(3) # Courte pause si limite atteinte
    except:
        pass
    return None
...
for index, row in df.iterrows():
    ...
    # --- AJOUT : RÉCUPÉRATION DU PRIX ---
    prix_affiche = recuperer_prix_haut(id_discogs)
    
    # Si Discogs ne renvoie rien, on regarde si un prix est écrit dans les commentaires de l'Excel
    if not prix_affiche:
        coms = str(row.get('Prix Haut - Commentaires', '')).strip()
        # Petit re_match pour chasser un nombre si vous l'aviez écrit manuellement
    ...
    viny_data = {
        ...
        "prix": prix_affiche, # <--- AJOUT : Transmis au JSON
        "comment": str(row.get('Prix Haut - Commentaires', ''))
    }
    collection.append(viny_data)
    time.sleep(0.2) # Légère pause pour respecter l'API Discogs (limite de requêtes)
...
# =====================================================================
# MODIFICATIONS CSS : Ajout de .badge-prix, et ajustement de .badge-qte à gauche pour un rendu propre
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: sans-serif; }
        ...
        /* MODIFICATION : Quantité basculée en HAUT À GAUCHE */
        ...
        /* AJOUT : Nouvelle pastille de Prix en HAUT À DROITE */
        .badge-prix { position: absolute; top: 8px; right: 8px; background: #e11d48; color: #ffffff; font-size: 11px; font-weight: 800; padding: 4px 8px; border-radius: 12px; border: 2px solid #ffffff; box-shadow: 0 4px 8px rgba(0,0,0,0.3); z-index: 10; font-family: sans-serif; }
...
html_fin = """
        ...
        function renderGrid() {
            ...
            document.getElementById('vinylGrid').innerHTML = filtered.map(item => {
                ...
                // MODIFICATION : Ajout de la structure HTML pour afficher la pastille de prix si existante
...
print("\n🎉 Parfait ! Tout est regroupé. Les boutons de type, les genres et les pastilles de prix haut sont synchronisés.")