...
<body>
    <header>
        <div class="global-counter">Total collection : <span id="totalCounter">{total_vinyles}</span></div>
...
# Assemblage final et écriture du fichier index.html
html_complet = html_debut + f"\nconst vinylData = {json_data};\nconst genresAuto = {json_genres};\nconst typesAuto = {json_types};\n" + html_fin

with open("index.html", "w", encoding="utf-8") as f:
    f.write(html_complet)
    
    # =====================================================================
# CRÉATION AUTOMATIQUE DE LA PAGE WANTED.HTML
# =====================================================================
...
<body>
    ...
    <script>
        ...
        document.getElementById('wantedGrid').innerHTML = wantedData.map(item => {{
            ...
            const commentBlock = item.comment ? '<div class="wanted-comment">' + item.comment + '</div>' : '';
...
print("🎉 Page Wanted.html générée avec succès !")

print(f"🎉 Terminé ! Fichier index.html généré avec succès. Total : {total_vinyles}")